...
function renderValidationInterpretationNote() {
  return `
    <div class="validation-note">
      ...
      <b>本次新增内容：</b>排行榜新增 IC胜率、超额年化、超额回撤、10组单调性、月均换手等排序列，便于直接按稳定性、相对收益、回撤控制和交易拥挤度筛选因子。
      Top30超额年化表示 Top30 组合相对基准月收益的年化结果，主要看因子是否提供市场之外的增量收益；Top30超额回撤表示这条超额收益曲线的最大回撤，数值越低说明相对基准的回撤压力越小。
      月均换手和年化换手用于衡量交易频率，收益接近时优先关注换手较低、成本压力较小的因子。
      10组收益柱状图用于观察从低分组到高分组的收益排序是否清晰，柱子越接近单调递增或递减，因子排序越有经济含义。
      36个月滚动 IC_IR 曲线用于观察信号稳定性是否随时间衰退或阶段性失效，若长期在 0 附近或频繁转负，需要谨慎使用。
      分层 IC 热力图用于比较因子在市值、流动性、行业等分层中的有效性，若颜色只集中在少数分层，说明因子更适合限定适用范围或配合约束使用。
