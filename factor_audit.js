...
function closeDetail() {
  ...
  if (resolveTrigger()) {
    // Restore immediately after removing inert; keep animation-frame and delayed retries for
    // browsers that move focus out of a newly hidden drawer asynchronously.
    restoreFocus();
    ...
    [100, 400].forEach(delay => setTimeout(() => {
      if (!document.activeElement || document.activeElement === document.body || drawer.contains(document.activeElement)) {
        restoreFocus();
      }
    }, delay));
  }
